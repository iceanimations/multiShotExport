...
class Shot(object):
    ...
    def exportPreview(self):
        ...
        try:
            ...
            subprocess.call('\"'+ osp.join(imgMgcPath, 'ffmpeg.exe') +'\" ' + ' -framerate ' + fps +' -start_number '+ str(self.startFrame) + ' -i '+ osp.join(jpgPath, self.getCameraNiceName() + '.%05d.jpg') +' -c:v libx264 -g 1 -bf 0 '+ movPath, shell=True)
            # add extracted audio
